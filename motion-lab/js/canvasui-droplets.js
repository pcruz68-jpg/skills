/* Canvas UI Droplets — bundled from the official vanilla source: https://canvasui.dev/docs/components/droplets
   Adapted to accept a pre-painted Canvas2D source when html-in-canvas is unavailable. */
...
function createDroplets(elements, options = {}) {
  ...
  function uploadContent() {
    if (!(htmlInCanvas || prepaintedSource) || !contentDirty) return;
    contentDirty = false;
    gl.bindTexture(gl.TEXTURE_2D, contentTexture);
    gl.texImage2D(
      gl.TEXTURE_2D,
  ...
  function render(timeSec) {
    ...
    gl.uniform1f(uniforms.uHasContent, htmlInCanvas || prepaintedSource ? 1 : 0);
