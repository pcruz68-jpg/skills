/* Canvas UI Cloth — bundled from the official vanilla source: https://canvasui.dev/docs/components/cloth
   Adapted to accept a pre-painted source canvas when html-in-canvas is unavailable. */

// ../../../../private/tmp/canvasui-build/rect-cache.ts
...
// ../../../../private/tmp/canvasui-build/Cloth/ClothVanilla.ts
var DEFAULTS = {
  pin: "top",
  wind: 3,
  speed: 0.5,
...
var SHADOW_VERT = `#version 300 es
precision highp float;
layout(location = 0) in vec2 aGrid;
layout(location = 1) in vec4 aData;
layout(location = 2) in vec2 aOffset;
...
function createCloth(elements, options = {}) {
  ...
  capture = () => {
    ...
    try {
      ...
      if (htmlInCanvas) sourceCtx.reset();
      wake();
    } catch {
    }
  };
  function syncCanvasSize() {
    ...
    contentMaxX = htmlInCanvas ? Math.min(
      1,
  ...
  function stepSim(dt) {
    ...
    for (let b = 0; b < NODES; b++) {
      rowForce[b] = Math.sin(kb1 * b - w1 * t + drift) + 0.45 * Math.sin(kb2 * b + w2 * t * 0.8 + 3);
    }
    for (let a = 0; a < NODES; a++) {
  ...
  function frame(now) {
    ...
    const delta = Math.min((now - lastTime) / 1e3, 1 / 20);
    lastTime = now;
    ...
    render();
    if (reducedMotion || config.wind <= 1e-3 && fieldEnergy < 4e-3 && touch.s < 0.01) {
      running = false;
      return;
    }
